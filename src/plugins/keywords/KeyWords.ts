...
let guildKeyWordMap: GuildKeyWordMap = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), "keywords.json"), "utf-8"));

setInterval(() => {
    try {
        fs.writeFileSync(path.resolve(process.cwd(), "keywords.json"), JSON.stringify(guildKeyWordMap));
    } catch(e) {
        console.log(e);
    }
}, 1000);
...
export const addPhrase: Command = {
    ...
    .addStringOption(new SlashCommandStringOption().setRequired(true).setName("phrase").setDescription("The phrase to add.")),
...
export const deletePhrase: Command = {
    ...
    .addStringOption(new SlashCommandStringOption().setRequired(true).setName("phrase").setDescription("The phrase to delete.")),
    handler: async (interaction: CommandInteraction) => {
        const guild = interaction.guild;
        if(!guild) return;
        const user = interaction.user;
