...
function Log(level:LogLevel, ...args: any[]) {
  ...
  processors.forEach(processor => processor(uncoloredOutputString, level, args));
  const colorizedOutputString = `\x1b[${getColor(level)}m[${time}]\x1b[0m ${coloredArgStrings.join(" ")}`;
  switch(level) {
    case LogLevel.log:
