...
describe("Uptime slash command", () => {
  test("Assert body", () => {
    ...
    expect(interaction.reply).toHaveBeenCalledWith(`I've been online for 1 minute`);
    jest.spyOn(process, "uptime").mockReturnValue(120000 + 4000 + (60 * 60 * 24 * 2 * 1000));
    interaction.reply.mockReset();
    uptime.handler(interaction as any);
    expect(interaction.reply).toHaveBeenCalledWith(`I've been online for 2 days, 2 minutes, 4 seconds`);
  })
})
